...
# Well-known SIDs that have no queryable directory object, so an LDAP
# objectSid lookup cannot resolve them. Mapped to their friendly ACL display
# name so reports show names instead of raw SIDs. Domain-relative groups
# (Domain Admins, etc.) are NOT listed here -- they resolve via LDAP.
WELL_KNOWN_SIDS = {
    "S-1-0-0": "Null SID",
    "S-1-1-0": "Everyone",
    "S-1-2-0": "Local",
...
class ADConnector:
    ...
    def resolve_sid(self, sid_str):
        ...
        if not sid_str:
            return sid_str
        # Well-known SIDs have no queryable directory object -- map them to a
        # friendly name first (also avoids a pointless LDAP round-trip).
        well_known = WELL_KNOWN_SIDS.get(str(sid_str))
        if well_known:
            return well_known
        if not self.ldap_conn:
            return sid_str
