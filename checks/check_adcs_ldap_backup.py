""" checks/check_adcs_ldap_backup.py - ADCS / PKI Vulnerability Check (LDAP BACKUP)
This is a backup of the original combined LDAP + Certipy ADCS check preserved before
check_adcs.py was updated to use Certipy exclusively.

To restore: copy this file over check_adcs.py.

Execution order (original):
  1. LDAP checks -- always run; enumerate CAs, templates, and flag ESC1-16 via direct
     LDAP queries against the PKI Configuration NC.
  2. Certipy check -- run only when Certipy is installed and connector credentials are
     available; performs a full certipy-ad find -vulnerable pass and merges results
     with the LDAP findings.
...
Prerequisites for Certipy section:
  ...
  Artifact saved to: Reports/Artifacts/adscan_certipy_<domain>_<timestamp>.json
"""
import glob
import json
import os
...
_TEMPLATE_ATTRS = [
    ...
    "nTSecurityDescriptor", "msPKI-Minimal-Key-Size", "flags", "objectClass",
]
_CA_ATTRS = [
...
_LDAPS_ERROR_PATTERNS = (
    "ssl wrapping error", "ssl: wrong version number", "ssl: unsupported protocol",
    "ssl handshake", "handshake failure", "ssleoferror", "certificate verify failed",
    "tlsv1 alert", "tlsv1.3 alert",
    "connection reset by peer", "[errno 104]", "[errno 111]", "[errno 110]",
    "error: socket", "got error: socket", "ldaps connection", "port 636",
)
...
def _run_certipy(creds, cwd=None, scheme=None):
    ...
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=120, cwd=cwd)  # nosec B603
...
def _run_ldap_checks(connector, verbose=False):
    ...
    if esc6_cas:
        findings.append({
            ...
            "severity": "critical", "deduction": 20,
            "description": (
                ...
                "regardless of template configuration."
            ),
            "recommendation": (
    ...
    esc1_templates = []; esc2_templates = []; esc3_templates = []
    esc9_templates = []; esc13_templates = []; esc15_templates = []
    weak_key_templates = []
    ...
    for tmpl in tmpl_entries:
        ...
        has_client_auth  = (_EKU_CLIENT_AUTH in tmpl_ekus or _EKU_SMART_CARD_AUTH in tmpl_ekus
                            or _EKU_ANY in tmpl_ekus or len(tmpl_ekus) == 0)
    ...
    cas_without_epa = [
        ...
        if (int(_get_str(ca, "flags") or 0) & _CA_FLAG_EPA_DISABLED) or not _get_str(ca, "flags")
    ]
    if cas_without_epa:
        findings.append({
    ...
    findings.append({
        ...
        "recommendation": (
            ...
            "StrongCertificateBindingEnforcement = 2. Apply KB5014754."
        ),
        "details": [
    ...
    if esc16_cas:
        findings.append({
            ...
            "description": (
                ...
                "suppressing the szOID_NTDS_CA_SECURITY_EXT extension from ALL issued certificates."
            ),
            "recommendation": (
    ...
    if esc1_templates:
        findings.append({"title": "ESC1: Certificate Templates Allow Enrollee-Supplied SAN",
            "severity": "critical", "deduction": 20,
            "description": (f"{len(esc1_templates)} certificate template(s) allow the enrollee to specify "
                "an arbitrary SAN and have a client authentication EKU without requiring manager approval."),
            "recommendation": "Disable CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT_ALT_NAME on all templates where it is not required.",
            "details": esc1_templates})
    if esc2_templates:
        findings.append({"title": "ESC2: Any-Purpose / SubCA Certificate Templates with Low-Privilege Enrollment",
            "severity": "critical", "deduction": 20,
            "description": (f"{len(esc2_templates)} template(s) have an Any-Purpose EKU or no EKU (SubCA) "
                "configured without manager approval or RA signature requirements."),
            "recommendation": "Remove Any-Purpose EKUs; add manager approval or RA signature requirements.",
            "details": esc2_templates})
    if esc3_templates:
        findings.append({"title": "ESC3: Enrollment Agent Templates Allow Certificate Request Delegation",
            "severity": "high", "deduction": 15,
            "description": (f"{len(esc3_templates)} template(s) grant the Certificate Request Agent EKU "
                "without requiring RA signatures or manager approval."),
            "recommendation": "Require at least 1 RA (Authorised Signature) on enrollment agent templates.",
            "details": esc3_templates})
    if esc9_templates:
        findings.append({"title": "ESC9: Templates With CT_FLAG_NO_SECURITY_EXTENSION",
            "severity": "high", "deduction": 15,
            "description": (f"{len(esc9_templates)} template(s) have CT_FLAG_NO_SECURITY_EXTENSION set."),
            "recommendation": "Remove CT_FLAG_NO_SECURITY_EXTENSION (0x80000) from msPKI-Certificate-Name-Flag.",
            "details": esc9_templates})
    if esc13_templates:
        findings.append({"title": "ESC13: Templates With Issuance Policy OIDs (Verify OID-Group Links)",
            "severity": "medium", "deduction": 8,
            "description": (f"{len(esc13_templates)} template(s) contain non-EKU policy OIDs. "
                "ESC13 occurs when an issuance policy OID is linked to a privileged AD group."),
            "recommendation": "Audit all issuance policy OIDs and verify OID-to-group links via msDS-OIDToGroupLink.",
            "details": esc13_templates})
    if esc15_templates:
        findings.append({"title": "ESC15: Schema V1 Templates May Lack NTDS Security Extension",
            "severity": "medium", "deduction": 8,
            "description": (f"{len(esc15_templates)} Schema V1 certificate template(s) support client authentication."),
            "recommendation": "Migrate V1 templates to V4; enforce StrongCertificateBindingEnforcement=2 via KB5014754.",
            "details": esc15_templates})
    if weak_key_templates:
        findings.append({"title": "Certificate Templates With Weak RSA Key Sizes (< 2048-bit)",
            "severity": "medium", "deduction": 8,
            "description": (f"{len(weak_key_templates)} certificate template(s) allow RSA key sizes below 2048 bits."),
            "recommendation": "Update msPKI-Minimal-Key-Size to at least 2048 on all templates.",
            "details": weak_key_templates})

    findings.append({
        "title": "ADCS Enrollee ACL Review Required", "severity": "info", "deduction": 0,
        "description": (
            f"ADScan found {len(tmpl_entries)} certificate template(s). "
            "A full ESC1/ESC4 assessment requires parsing the nTSecurityDescriptor of each template. "
        ...
        "details": [f"Total templates enumerated: {len(tmpl_entries)}"] + [f"CA: {_get_name(ca)}" for ca in ca_entries],
    })
    return findings

# ---------------------------------------------------------------------------
# Certipy-based ADCS check
# ---------------------------------------------------------------------------
def _run_certipy_check(connector, verbose=False):
    ...
    try:
        ...
        findings.append({
            ...
            "severity": "info", "deduction": 0,
            "description": f"Could not extract required credentials from the connector: {e}.",
            "recommendation": "Ensure the ADScan connector exposes: username, password, domain, dc_host.",
            "details": [str(e)],
        })
        return findings

    log.debug("  [Certipy] Running as %s@%s against DC %s", creds['username'], creds['domain'], creds['dc_ip'])
    artifacts_dir = os.path.abspath(getattr(connector, "artifacts_dir", os.path.join("Reports", "Artifacts")))
    ...
    try:
        ...
        if _is_ldaps_error(stdout, stderr):
            ...
            returncode, stdout, stderr = _run_certipy(creds, cwd=artifacts_dir, scheme="ldap")
        ...
        if dbg:
            ...
            dbg.log_subprocess(
                cmd=["certipy-ad", "find", "-u", upn, "-p", "<redacted>",
                     "-dc-ip", creds["dc_ip"], "-enabled", "-vulnerable"]
        ...
        if parsed:
            if ldap_fallback_used:
                parsed.insert(0, {
                    ...
                    "recommendation": (
                        ...
                        "or SAN matches the DC FQDN. "
                        "Verify with: openssl s_client -connect <DC_IP>:636"
                    ),
                    "details": [f"DC: {creds['dc_ip']}", "LDAPS error detected - fallback to port 389 used."],
                })
            ...
            findings.append({
                ...
                "recommendation": "Continue to monitor ADCS configuration. Re-run periodically or after any PKI changes.",
        ...
        findings.append({
            ...
            "severity": "info", "deduction": 0,
            "description": "Certipy did not complete within the 120-second timeout.",
...
def run_check(connector, verbose=False):
    """Run combined LDAP + Certipy ADCS checks (BACKUP - original logic preserved).

    Phase 1: LDAP-based checks (always executed).
    Phase 2: Certipy-based checks (when Certipy is installed and credentials available).
    Certipy wins deduplication over LDAP for any ESC it reports.
    """
    import re as _re
    ...
    try:
        ...
        ldap_findings = [{
            "title": "ADCS LDAP Check Failed", "severity": "info", "deduction": 0,
    ...
    try:
        ...
        certipy_findings = [{
            "title": "ADCS Certipy Check Failed", "severity": "info", "deduction": 0,
    ...
    merged = [f for f in ldap_findings if _esc_key(f["title"]) not in certipy_esc_keys]
